...
public class SocketClientHandler implements Runnable {
	...
	HttpGetDataInterface getDataInterface = null;
	public SocketClientHandler(Socket client, HttpGetDataInterface gdataInterface) {
		getDataInterface = gdataInterface;
		this.client = client;
	}
	...
	private void readResponse() throws IOException, InterruptedException {
		...
		try {
			...
			String [] array = requestHeader.split("\n");
			if (array[0].contains("GET")
					/*&& checkURL(file)*/) {
				...
				if (getDataInterface!=null) {
					response.write(getDataInterface.responceData(array[0].replaceFirst("GET ", "").trim()));
				}else {
					response.write("<html>No getDataInterface</html>");	
				}
				sb.setLength(0);
				response.flush();
